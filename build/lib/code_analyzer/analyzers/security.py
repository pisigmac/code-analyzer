...
class SecurityAnalyzer(BaseAnalyzer):
    ...
    def _check_sql_injection(self, node: ast.Call) -> None:
                ...
                if len(node.args) >= 1:
                    ...
                    if self._contains_string_formatting(first_arg):
                        fix = Fix(
                            ...
                            replacement_code="# Use parameterized query: cursor.execute('SELECT * FROM users WHERE id = %s', (user_id,))",
                            confidence=0.8,
        ...
        # Also check for string formatting in any context that looks like SQL
        if len(node.args) >= 1:
            first_arg = node.args[0]
            if self._contains_string_formatting(first_arg):
                arg_str = ast.unparse(first_arg).lower()
                sql_keywords = ["select", "insert", "update", "delete", "drop", "create"]
                if any(kw in arg_str for kw in sql_keywords):
                    fix = Fix(
                        description="Use parameterized queries",
    ...
    def _check_sql_injection_fstring(self, node: ast.JoinedStr) -> None:
        """Check for SQL injection in f-strings."""
        if not self.is_rule_enabled("detect_sql_injection"):
            return
